...
while (( "$#" )); do
    ...
    if [ "$1" = "planet" ]; then
        ...
        $PYTHON_EXE -m wwwportalmlekozyjestart.scripts.planet --init 30 --download --update 1
    fi
    ...
    if [ "$1" = "init" ]; then
        ...
        $PYTHON_EXE $DJANGO_ADMIN migrate wwwportalmlekozyjestart.server --settings=$DJANGO_SETTINGS_MODULE
    ...
    if [ "$1" = "migrate" ]; then
        echo "*** MIGRATE_PATH=$MIGRATE_PATH"
        echo "*** MIGRATE_LIMIT=$MIGRATE_LIMIT"
        $PYTHON_EXE -m wwwportalmlekozyjestart.scripts.migrate --path $MIGRATE_PATH --limit $MIGRATE_LIMIT
    fi

    if [ "$1" = "index" ]; then        
        echo "*** indexing all post content"
        $PYTHON_EXE -m wwwportalmlekozyjestart.server.search
    fi
