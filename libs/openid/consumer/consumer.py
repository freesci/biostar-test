...
class GenericConsumer(object):
    ...
    def _checkReturnTo(self, message, return_to):
        ...
        try:
            ...
            logging.info("Verifying return_to arguments: %s" % (why[0],))
            return False
    ...
    def _verifyDiscoveryResultsOpenID1(self, resp_msg, endpoint):
        ...
        if endpoint is not None:
            try:
                ...
                logging.info("Error attempting to use stored discovery information: " +
                            str(e))
                logging.info("Attempting discovery to verify endpoint")
            else:
                return endpoint
    ...
    def _verifyDiscoveredServices(self, claimed_id, services, to_match_endpoints):
        ...
        for endpoint in services:
            ...
            logging.info('Discovery verification failure for %s' %
                        (claimed_id,))
            for failure_message in failure_messages:
                logging.info(' * Endpoint mismatch: ' + failure_message)
    ...
    def _checkAuth(self, message, server_url):
        ...
        try:
            ...
            logging.info('check_authentication failed: %s' % (e[0],))
            return False
        else:
    ...
    def _processCheckAuthResponse(self, response, server_url):
        ...
        if invalidate_handle is not None:
            ...
            if self.store is None:
                logging.info('Unexpectedly got invalidate_handle without '
                            'a store!')
            else:
        ...
        if is_valid == 'true':
            ...
            logging.info('Server responds that checkAuth call is not valid')
            return False
        ...
        try:
            ...
            if supportedTypes is not None:
                ...
                try:
                    ...
                    logging.info('Server %s refused its suggested association '
    ...
    def _extractSupportedAssociationType(self, server_error, endpoint,
        ...
        if assoc_type is None or session_type is None:
            logging.info('Server responded with unsupported association '
            ...
            logging.info(fmt % (session_type, assoc_type))
            return None
        else:
    ...
    def _requestAssociation(self, endpoint, assoc_type, session_type):
        ...
        try:
            ...
            logging.info('openid.associate request failed: %s' % (why[0],))
            return None

        try:
            assoc = self._extractAssociation(response, assoc_session)
        except KeyError, why:
            logging.info('Missing required parameter in response from %s: %s'
                        % (endpoint.server_url, why[0]))
            return None
        except ProtocolError, why:
            logging.info('Protocol error parsing response from %s: %s' % (
                endpoint.server_url, why[0]))
            return None
        else:
            return assoc
    ...
    def _getOpenID1SessionType(self, assoc_response):
        ...
        if session_type == 'no-encryption':
            logging.info('WARNING: OpenID server sent "no-encryption"'
                        'for OpenID 1.X')
...
class AuthRequest(object):
    ...
    def getMessage(self, realm, return_to=None, immediate=False):
        ...
        msg = "Generated %s request to %s %s" % (mode, self.endpoint.server_url, assoc_log_msg)
