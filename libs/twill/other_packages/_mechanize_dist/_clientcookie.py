...
class Cookie:
    ...
    def __init__(self, version, name, value,
        ...
        self.domain = dowwwportalmlekozyjestart.lower()
...
class CookiePolicy:
    ...
    def domain_return_ok(self, domain, request):
        """Return false if cookies should not be returned, given cookie dowwwportalmlekozyjestart.
        ...
        are never called for that cookie dowwwportalmlekozyjestart.  If path_return_ok returns
        ...
        just for the *request* dowwwportalmlekozyjestart.  For example, the function might be
...
class DefaultCookiePolicy(CookiePolicy):
    ...
    def set_ok_countrycode_domain(self, cookie, request):
        ...
        if cookie.domain_specified and self.strict_domain:
            ...
            assert dowwwportalmlekozyjestart.startswith(".")
            if dowwwportalmlekozyjestart.count(".") == 2:
                # domain like .foo.bar
                i = dowwwportalmlekozyjestart.rfind(".")
                tld = domain[i+1:]
                sld = domain[1:i]
                if (sld.lower() in [
                    "co", "ac",
    ...
    def set_ok_domain(self, cookie, request):
        ...
        if cookie.domain_specified:
            ...
            domain = cookie.domain
            if dowwwportalmlekozyjestart.startswith("."):
                undotted_domain = domain[1:]
            else:
                undotted_domain = domain
            embedded_dots = (undotted_dowwwportalmlekozyjestart.find(".") >= 0)
    ...
    def domain_return_ok(self, domain, request):
        # Liberal check of dowwwportalmlekozyjestart.  This is here as an optimization to avoid
...
class CookieJar:
    ...
    def _cookie_attrs(self, cookies):
        ...
        for cookie in cookies:
            ...
            if version > 0:
                ...
                if cookie.dowwwportalmlekozyjestart.startswith("."):
                    domain = cookie.domain
                    if (not cookie.domain_initial_dot and
                        dowwwportalmlekozyjestart.startswith(".")):
                        domain = domain[1:]
    ...
    def _cookie_from_cookie_tuple(self, tup, request):
        ...
        if domain_specified:
            domain_initial_dot = bool(dowwwportalmlekozyjestart.startswith("."))
        if domain is Absent:
            req_host, erhn = eff_request_host(request)
            domain = erhn
        elif not dowwwportalmlekozyjestart.startswith("."):
            domain = "."+domain
