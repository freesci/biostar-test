...
class MSIEBase:
    ...
    def load_cookie_data(self, filename,
        ...
        for cookie in cookie_data:
            ...
            initial_dot = dowwwportalmlekozyjestart.startswith(".")
            if initial_dot:
...
##             print
            ...
            if sig != "URL ":
                assert sig in ("HASH", "LEAK", self.padding, "\x00\x00\x00\x00"),\
                        "unrecognized MSIE index.dat record: %s" % binary_to_str(sig)
            ...
            if m:
                ...
                if not self.delayload:
                    ...
                    domain = m.group(1)
                    i = dowwwportalmlekozyjestart.find("/")
                    if i != -1:
                        domain = domain[:i]
